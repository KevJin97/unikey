...
{
	this->queue[this->tail.fetch_add(1, std::memory_order_acq_rel) % SIZE].store(data, std::memory_order_release);
}

void* Cyclic_Queue::pop()
{
	return (this->size()) ?
		this->queue[this->head.fetch_add(1, std::memory_order_acq_rel) % SIZE].exchange(nullptr, std::memory_order_acq_rel)
		:
		nullptr;
}
