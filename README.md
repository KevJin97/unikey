...
## Compile The Project
```bash
cmake -S . -B build -DCMAKE_BUILD_TYE=Release && cmake --install build
```
or using ninja as the generator
```bash
cmake -S . -B build -G Ninja -DCMAKE_BUILD_TYPE=Release && cmake --install build
```
...
```bash
sudo setcap "cap_setgid=eip" ./install/unikey
```
